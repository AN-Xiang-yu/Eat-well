class Ingredient {
  ...
  async getNomsIngredients() {
    return this.sequelize.query(`SELECT nom as nomIngredient, id AS idIngredient FROM ingredient`).catch((err) => res.status(400).json({ error: err }))
  }
  ...
  async getIdIngredients() {
    return this.sequelize.query(`SELECT id FROM ingredient`).catch((err) => res.status(400).json({ error: err }))
  }
